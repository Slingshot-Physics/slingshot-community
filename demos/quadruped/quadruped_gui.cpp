...
void QuadrupedGuiCallback::operator()(void)
{
...
bool QuadrupedGuiCallback::no_window(void)
{
   ...
   bool gridlines_pressed = ImGui::Checkbox("Show grid lines", &state_.show_grid);
   ...
   ui_modified |= ImGui::DragFloat(
      "Hip rotation", &state_.hip_rotation_angle_setpoint, 0.01f, -M_PI_2, M_PI_2
   );

   ui_modified |= ImGui::DragFloat(
      "Hip flexion", &state_.hip_flexion_angle_setpoint, 0.01f, -M_PI_2, M_PI_2
   );

   ui_modified |= ImGui::DragFloat(
      "Knee flexion", &state_.knee_flexion_angle_setpoint, 0.01f, -M_PI_2, M_PI_2
   );
