#include "shape_feature_isometric_ui.hpp"
...
bool MeshFeature::no_window(void)
{
   ...
   if (ui_modified_)
   {
      shape_loader.trans_B_to_W().scale = identityMatrix();
   }
   ...
   if (ui_modified_)
   {
      ...
      geometry::types::isometricTransform_t iso_trans_B_to_W = {
         shape_loader.trans_B_to_W().rotate,
         shape_loader.trans_B_to_W().translate
      };
